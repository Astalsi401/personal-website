...
export type DemoFrameProps = {
  ...
  scssHref?: string;
  js?: string[];
  lib?: "react" | "d3js" | "jquery" | "leaflet";
  template?: "grid-center" | "flex-center";
  bgColor?: string;
};
...
export const DemoFrame: React.FC<DemoFrameProps> = ({
    ...
    if (data.load) {
      const { css } = scssHref
        ? await sass.compileStringAsync(
            await fetch(scssHref).then((res) => res.text())
          )
        : { css: "" };
      const templateCss = template
        ? await fetch(
            ...
            }/assets/demo-files/templates/${template}.css`
          ).then((res) => res.text())
        : null;
      const scripts = js
        ? await Promise.all(
            js.map((path) => fetch(path).then((res) => res.text()))
          )
        : [];
  ...
  useEffect(() => {
    ...
    iframeRef.current?.addEventListener("load", handleSrcIframe, {
      once: true,
      signal,
    });
  ...
  return (
    <div className="demo-frame my-2 p-2 pt-0 position-relative rounded-1 border-solid border-1 bd-primary-300">
      <a
        href="#"
        className={clsx(
          "full-page mb-1 ps-2 pe-4 d-block position-relative float-end text-small text-primary text-decoration-none w-max",
          isActive(fullPage)
        )}
        onClick={handleClick}
      >
      ...
      <iframe
        className={clsx("w-100 border-0 bg-code-text", isActive(fullPage))}
        style={{ height }}
        src={
          src
            ? src
            : `${import.meta.env.BASE_URL}/assets/demo-files/index-${
                lib ? lib : ""
              }.html`
        }
        ref={iframeRef}
