...
export const getCategories: () => Promise<Categories> = async () => {
  ...
  return { category: root, href, pages: index.map((d) => ({ page: d.category, href: d.href, icon: d.icon })) };
};
export const getPages = async ({ params: { href } }: { params: Params }) => {
  const { index } = await getIndex();
  return index.find((d) => d.href === `/${href}`);
};
export const getSections = async ({ params: { href, page } }: { params: Params }) => await Promise.all([getIndex(), import(`@/pages/${href}/${page}/page.tsx`) as Promise<Post>]).then(([{ index }, { Sections }]) => ({ title: index.find((d) => d.href === `/${href}`)?.pages?.find((d) => d.href === `/${page}`)?.page, Sections }));
