import{j as e}from"./index-t9TkrohJ.js";import{C as i}from"./codeChunk-RNcgW44A.js";const r=[{title:"Mann Whitney U Test",content:e.jsxs(e.Fragment,{children:[e.jsxs("ol",{children:[e.jsx("li",{children:"適用範圍："}),e.jsxs("ul",{children:[e.jsx("li",{children:"此檢驗可看作為t test的替代。當後者違反所謂的equal variance時，可用Mann Whitney U Test作為替代。"}),e.jsx("li",{children:"此檢驗屬於nonparametric test，不需要假定母體的分佈呈常態分配。"}),e.jsx("li",{children:"即令如此，此假定仍然包含樣本來自隨機抽樣的假定。"}),e.jsx("li",{children:"適用於ordinal data，比較兩組之間的差距。"})]}),e.jsx("li",{children:"作法："}),e.jsxs("ol",{type:"i",children:[e.jsx("li",{children:"先對樣本中的資訊進行排序"}),e.jsxs("li",{children:["在下列公式中找尋較小值的U",e.jsx("img",{src:"data:image/png;base64,",width:"254px",heigh:"105px"}),e.jsxs("ul",{children:[e.jsxs("li",{children:[e.jsxs("i",{children:["N",e.jsx("sub",{children:"1"})]}),"與",e.jsxs("i",{children:["N",e.jsx("sub",{children:"2"})]}),"為各組的個案數"]}),e.jsxs("li",{children:["Σ",e.jsxs("i",{children:["R",e.jsx("sub",{children:"1"})]}),"與Σ",e.jsxs("i",{children:["R",e.jsx("sub",{children:"2"})]}),"為第一組與第二組的排序總和"]})]})]}),e.jsxs("li",{children:["取較小的U值，用於下列公式中，去取得Z值",e.jsx("img",{src:"data:image/png;base64,",width:"157",heigh:"72"})]}),e.jsx("li",{children:"若Z值大於1.96，則意味著虛無假設遭到拒絕，反之，則保留虛無假設"})]})]}),e.jsxs("div",{className:"my-2",children:[e.jsx("div",{className:"text-bold text-large",children:"範例："}),e.jsx(i,{code:`clear
input g stress rank
1 12 10
1 15 11
1 4  1
1 7  3
1 8  5
1 16 12
1 20 15
1 10 8
1 8  5
2 23 17
2 11 9
2 24 18
2 18 13.5
2 18 13.5
2 6  2  
2 9  7
2 8  5
2 21 16
end

ranksum stress, by(g)`,lang:"stata"}),e.jsx(i,{code:`. clear

. input g stress rank

             g     stress       rank
  1. 1 12 10
  2. 1 15 11
  3. 1 4  1
  4. 1 7  3
  5. 1 8  5
  6. 1 16 12
  7. 1 20 15
  8. 1 10 8
  9. 1 8  5
 10. 2 23 17
 11. 2 11 9
 12. 2 24 18
 13. 2 18 13.5
 14. 2 18 13.5
 15. 2 6  2  
 16. 2 9  7
 17. 2 8  5
 18. 2 21 16
 19. end

. 
. ranksum stress, by(g)

Two-sample Wilcoxon rank-sum (Mann-Whitney) test

           g |      obs    rank sum    expected
-------------+---------------------------------
           1 |        9          70        85.5
           2 |        9         101        85.5
-------------+---------------------------------
    combined |       18         171         171

unadjusted variance      128.25
adjustment for ties       -0.66
                     ----------
adjusted variance        127.59

Ho: stress(g==1) = stress(g==2)
             z =  -1.372
    Prob > |z| =   0.1700`,lang:"output"})]})]})},{title:"Kruskal-Wallis Test",content:e.jsxs(e.Fragment,{children:[e.jsxs("ol",{children:[e.jsxs("li",{children:["適用範圍：",e.jsxs("ul",{children:[e.jsx("li",{children:"此檢驗可看作為ANOVA的替代。當後者違反所謂的equal variance時，可用Kruskal-Wallis Test作為替代。"}),e.jsx("li",{children:"此檢驗屬於nonparametric test，不需要假定母體的分佈呈常態分配。"}),e.jsx("li",{children:"即令如此，此假定仍然包含樣本來自隨機抽樣的假定。"}),e.jsx("li",{children:"適用於ordinal data，比較三組或以上組別之間的差距。"})]})]}),e.jsxs("li",{children:["做法：",e.jsxs("ol",{type:"i",children:[e.jsx("li",{children:"先對樣本中的資訊進行排序"}),e.jsxs("li",{children:["在下列公式中找尋較小值的H",e.jsx("img",{src:"data:image/png;base64,",width:"245px",heigh:"57px"}),e.jsxs("ul",{children:[e.jsxs("li",{children:[e.jsx("i",{children:"N"}),"為個案總數"]}),e.jsxs("li",{children:[e.jsxs("i",{children:["n",e.jsx("sub",{children:"i"})]}),"為各組的個案數"]}),e.jsxs("li",{children:["Σ(",e.jsxs("i",{children:["R",e.jsx("sub",{children:"i"})]}),")",e.jsx("sup",{children:"2"}),"是各組排序的總和"]})]})]}),e.jsx("li",{children:"若H值大於1.96，則意味著虛無假設遭到拒絕，反之則保留虛無假設"})]})]})]}),e.jsxs("div",{className:"my-2",children:[e.jsx("div",{className:"text-bold text-large",children:"範例："}),e.jsx(i,{code:`clear
input g reward
1 20
1 15
1 17
1 13
1 18
1 16
2 14
2 8
2 11
2 10
2 6
2 9
3 12
3 11
3 9
3 5
3 6
3 7
end`,lang:"stata"}),e.jsx(i,{code:`. clear

. input g reward

             g     reward
  1. 1 20
  2. 1 15
  3. 1 17
  4. 1 13
  5. 1 18
  6. 1 16
  7. 2 14
  8. 2 8
  9. 2 11
 10. 2 10
 11. 2 6
 12. 2 9
 13. 3 12
 14. 3 11
 15. 3 9
 16. 3 5
 17. 3 6
 18. 3 7
 19. end`,lang:"output"}),e.jsx("p",{children:"Stata算出H=11.038"}),e.jsx(i,{code:"kwallis reward, by(g)",lang:"stata"}),e.jsx(i,{code:`. kwallis reward, by(g)

Kruskal-Wallis equality-of-populations rank test

  +--------------------+
  | g | Obs | Rank Sum |
  |---+-----+----------|
  | 1 |   6 |    92.00 |
  | 2 |   6 |    44.50 |
  | 3 |   6 |    34.50 |
  +--------------------+

chi-squared =    11.038 with 2 d.f.
probability =     0.0040

chi-squared with ties =    11.072 with 2 d.f.
probability =     0.0039`,lang:"output"}),e.jsx("p",{children:"df=2 以組數來計算df"}),e.jsx(i,{code:"di invchi2tail(2, .05)",lang:"stata"}),e.jsx(i,{code:`. di invchi2tail(2, .05)
5.9914645`,lang:"output"})]})]})}];export{r as default};
